...
- **LiteLLM Chat** tool window, docked on the right. Replies stream token by token over SSE rather than appearing all at once, because a chat answer is long enough that waiting for the last token before showing the first is the difference between usable and not. **Stop** abandons a response and keeps what has already arrived. Code in an answer is rendered in a real read-only editor — the IDE's own syntax highlighting, not an approximation — with **Copy** and **Insert** next to it, since an answer whose code you cannot get into the file is only half an answer. The conversation lives in a project-level service, so hiding and reopening the tool window does not lose the thread. Gateways that refuse `stream: true` fall back to a buffered reply, probed once per gateway per session.
- Editor right-click action **Add Selection to LiteLLM Chat**: opens the chat and drops the selection into the input as a fenced block headed with `Foo.kt:12-30`. The text goes into the input rather than being sent, so the question can be typed around it and the attachment is visible and editable before anything leaves the machine. A selection that itself contains a Markdown fence gets a longer wrapper, or it would terminate the block early and the remainder would be read as prose.
- Chat model picker in the tool window, populated from the gateway's `/models` endpoint. It starts with the configured commit model and can be changed for the active chat without adding a duplicate Settings field.
